...
def check_internet_connection():
    """Check that we can actually reach GitHub before doing anything else.
    Returns True if online, False otherwise (with a helpful message printed)."""
    print("  Checking internet connection...")
    try:
        req = urllib.request.Request(
            'https://api.github.com',
            headers={'User-Agent': 'AtlasSync/1.0'},
            method='GET'
        )
        ...
        print(f"    ({reason})")
        print("  Please check your Wi-Fi / network connection and try again.")
        return False
...
def api_request(url, token, method='GET', data=None):
    ...
    try:
        ...
        print(f"\n  ✗ Lost internet connection while talking to GitHub ({e.reason}).")
        print("  Please check your connection and re-run the script.")
        sys.exit(1)
...
def main():
    ...
    # Check internet connection before asking for anything else
